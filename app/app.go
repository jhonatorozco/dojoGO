...
func create(w http.ResponseWriter, r *http.Request){
	...
	c := session.DB("dojogo").C("people")
	err = c.Insert(&Person{usuario.Name, usuario.Phone, usuario.Age, usuario.Gender})
	if err != nil {
					log.Fatal(err)
	}
...
func getByGender(w http.ResponseWriter, r *http.Request){
	vars := mux.Vars(r)
	gender := vars["genderUser"]
	session, err := mgo.Dial("mongodb:27017/dojogo")
	if err != nil {
					panic(err)
	}
	...
	var usuarios []Person
	c := session.DB("dojogo").C("people")
	err = c.Find(bson.M{"Gender": gender}).All(&usuarios)
	respuesta, err :=  json.Marshal(usuarios)
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
	w.Write(respuesta)
...
func main(){
	...
	r.HandleFunc("/user/gender/{genderUser}", getByGender).Methods("GET")
